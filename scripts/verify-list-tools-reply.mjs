...
async function run() {
  ...
  assert.equal(typeof POST, "function", "chat POST route 应该可用");
  ...
  const request = new Request("http://localhost/api/ai/chat", {
    ...
    body: JSON.stringify({
      projectId: "project_verify_list_tools",
  ...
  assert.match(text, /system\.listTools/, "应包含 system.listTools");

  console.log("verify_list_tools_reply_ok=true");
...
run().catch((error) => {
  console.error("verify_list_tools_reply_ok=false");
