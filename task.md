...
- [x] **并行块 W5-A：时间线数据模型（前置：W4-C）**
  1. [x] 创建 `entities/entity_aliases/events/event_entities/timeline_snapshots` 表。
  2. [x] 实现实体归一与别名映射 repository。
  3. [x] 实现事件版本字段与快照写入逻辑。
  4. [x] 实现冲突检测（时间顺序/重复事件/实体冲突）。

- [x] **并行块 W5-B：事件抽取工作流（前置：W5-A, W3-A）**
  1. [x] 实现 Mastra Workflow 抽取候选实体与事件（结构化 JSON）。
  2. [x] 实现代码侧校验、去重、排序、合并。
  3. [x] 实现低置信度事件入人工确认队列。
  4. [x] 实现章节改写触发“受影响事件重算”。

- [x] **并行块 W5-C：时间线接口（前置：W5-B）**
  1. [x] 实现 `POST /api/timeline/extract`。
  2. [x] 实现 `GET /api/timeline/entity/:entityId`。
  3. [x] 实现 `PATCH /api/timeline/events/:eventId`（confirm/reject/edit）。
  4. [x] 返回统一时间线事件结构（含 `confidence/status`）。

- [x] **并行块 W5-D：时间线前端（前置：W5-C, W2-B）**
  1. [x] 实现实体列表与实体详情页。
  2. [x] 实现按章节排序的时间线视图。
  3. [x] 实现低置信度事件审核 UI。
  4. [x] 实现事件编辑/确认/拒绝交互与状态刷新。
  5. [x] 验证“章节修改 -> 时间线自动更新”链路。

---

## W6 GraphRAG 增强波次（可选并行，依赖 W4）

- [x] **并行块 W6-A：GraphRAG 接入（前置：W4-C）**
  1. [x] 接入 `createGraphRAGTool` 并完成最小调用链路。
  2. [x] 建立关系查询专用检索入口。
  3. [x] 将 GraphRAG 结果纳入统一证据结构。

- [x] **并行块 W6-B：触发策略与防噪（前置：W6-A）**
  1. [x] 实现“仅关系型问题触发 GraphRAG”的策略开关。
  2. [x] 实现阈值与回退机制（噪声过高回退基线检索）。
  3. [x] 记录是否启用 GraphRAG 到响应字段。

- [x] **并行块 W6-C：A/B 评估（前置：W6-B, W4-D）**
  1. [x] 构建关系型查询测试集。
  2. [x] 对比基线检索与 GraphRAG 的召回率与噪声。
  3. [x] 输出是否默认启用 GraphRAG 的决策结论。

---

## W7 导入导出、快照与收尾波次（可并行，依赖 W5 + W6(可选)）

- [x] **并行块 W7-A：导入导出能力（前置：W5-C）**
  1. [x] 实现项目 JSON 导入导出。
  2. [x] 实现章节多格式导入（docx/pdf/epub 解析接口对接）。
  3. [x] 实现导入错误报告与可恢复提示。

- [x] **并行块 W7-B：快照系统（前置：W5-A）**
  1. [x] 实现自动快照（按编辑里程碑/间隔）。
  2. [x] 实现手动创建快照与恢复。
  3. [x] 实现快照差异展示（章节与时间线）。

- [x] **并行块 W7-C：测试与性能（前置：W7-A, W7-B）**
  1. [x] 完成核心单元测试（context-engine、RAG、timeline、SSE、审批状态机）。
  2. [x] 在 60s 超时约束下执行测试命令并修复失败项。
  3. [x] 完成静态检查与格式化校验。
  4. [x] 完成关键路径性能优化并记录前后对比。

- [x] **并行块 W7-D：文档与验收（前置：W7-C）**
  1. [x] 更新架构文档、接口文档、运行文档。
  2. [x] 跑通完整闭环验收：新建 -> 写作 -> AI -> 快照 -> 导出。
  3. [x] 输出发布前检查清单并完成最终勾选。
...
- [x] AI 流式生成稳定、可中断、可回填。
- [x] RAG 跨章节召回稳定且可返回证据。
- [x] 时间线可抽取、可审核、可追溯、可重算。
- [x] Tool 写操作均经过审批并可审计。
- [x] 完成闭环：新建 -> 写作 -> AI -> 快照 -> 导出。
