...
- [ ] **并行块 W2-A：项目与章节 API（前置：W1-A）**
  1. [ ] 实现 `GET/POST /api/projects`。
  2. [ ] 实现 `GET/POST /api/projects/:projectId/chapters`。
  3. [ ] 实现 `PATCH /api/chapters/:chapterId`。
  4. [ ] 加入请求校验与统一错误响应结构。
  5. [ ] 覆盖“新建作品 -> 新建章节 -> 编辑保存”最小链路测试。

- [ ] **并行块 W2-B：工作台三栏与导航（前置：W1-E）**
  1. [ ] 完成三栏布局与基础路由结构。
  2. [ ] 实现项目列表与章节列表视图。
  3. [ ] 实现创建项目/章节交互与状态反馈。
  4. [ ] 接入 Zustand 管理工作台状态。

- [ ] **并行块 W2-C：TipTap 编辑体验（前置：W1-E）**
  1. [ ] 集成 TipTap 基础扩展与最小工具条。
  2. [ ] 实现字数统计栏与保存状态提示。
  3. [ ] 接入快捷键（保存/基础编辑命令）。
  4. [ ] 与章节 API 打通自动保存/手动保存。
